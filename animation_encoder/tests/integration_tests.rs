...
fn test_minimal_animation() {
    let animation = Animation {
        ...
        frames: vec![
            Frame {
                time_ms: 0,
                leds: vec![RGB { r: 128, g: 128, b: 128 }; 20],
            },
        ],
        audio_notes: vec![],
    };
...
fn test_compression_efficiency() {
    let animation = Animation {
        ...
        frames: (0..100)
            .map(|i| Frame {
                time_ms: i * 50,
                leds: vec![RGB { r: (i % 256) as u8, g: 0, b: 0 }; 20],
            })
            .collect(),
        audio_notes: vec![
            AudioNote {
...
fn verify_audio_notes(data: &[u8], original: &Animation) {
    ...
    for (i, note) in original.audio_notes.iter().enumerate() {
        ...
        let duration_ms = cursor.read_u16::<LittleEndian>().unwrap();
        assert_eq!(duration_ms, note.duration_ms, "Note {} duration_ms mismatch", i);
        
        let frequency_hz = cursor.read_u16::<LittleEndian>().unwrap();
        assert_eq!(frequency_hz, note.frequency_hz, "Note {} frequency_hz mismatch", i);
