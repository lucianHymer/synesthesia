...
fn validate_animation(animation: &Animation) -> Result<(), EncodingError> {
    ...
    for note in &animation.audio_notes {
        ...
        if note.start_ms + note.duration_ms > animation.duration_ms {
            return Err(EncodingError::AudioTimingError(note.start_ms, note.duration_ms));
        }
    }
...
fn serialize_to_binary(animation: &Animation) -> Result<Vec<u8>, EncodingError> {
    ...
    buffer.write_u16::<LittleEndian>(header.version)
        .map_err(|e| EncodingError::SerializationError(e.to_string()))?;
    buffer.write_u16::<LittleEndian>(header.duration_ms)
        .map_err(|e| EncodingError::SerializationError(e.to_string()))?;
    buffer.write_u8(header.fps)
        .map_err(|e| EncodingError::SerializationError(e.to_string()))?;
    buffer.write_u8(header.reserved)
        .map_err(|e| EncodingError::SerializationError(e.to_string()))?;
    buffer.write_u16::<LittleEndian>(header.frame_count)
        .map_err(|e| EncodingError::SerializationError(e.to_string()))?;
    buffer.write_u16::<LittleEndian>(header.note_count)
        .map_err(|e| EncodingError::SerializationError(e.to_string()))?;
    
    for frame in &animation.frames {
        buffer.write_u16::<LittleEndian>(frame.time_ms)
            .map_err(|e| EncodingError::SerializationError(e.to_string()))?;
        
        for led in &frame.leds {
            buffer.write_u8(led.r)
                .map_err(|e| EncodingError::SerializationError(e.to_string()))?;
            buffer.write_u8(led.g)
                .map_err(|e| EncodingError::SerializationError(e.to_string()))?;
            buffer.write_u8(led.b)
                .map_err(|e| EncodingError::SerializationError(e.to_string()))?;
        }
    }
    
    for note in &animation.audio_notes {
        buffer.write_u16::<LittleEndian>(note.start_ms)
            .map_err(|e| EncodingError::SerializationError(e.to_string()))?;
        buffer.write_u16::<LittleEndian>(note.duration_ms)
            .map_err(|e| EncodingError::SerializationError(e.to_string()))?;
        buffer.write_u16::<LittleEndian>(note.frequency_hz)
            .map_err(|e| EncodingError::SerializationError(e.to_string()))?;
        buffer.write_u8(note.voice)
            .map_err(|e| EncodingError::SerializationError(e.to_string()))?;
        buffer.write_u8(note.waveform)
            .map_err(|e| EncodingError::SerializationError(e.to_string()))?;
        buffer.write_u8(0)
            .map_err(|e| EncodingError::SerializationError(e.to_string()))?;
    }
...
mod tests {
    ...
    fn test_audio_timing_error() {
        ...
        let result = encode_animation(&animation);
        assert!(matches!(result, Err(EncodingError::AudioTimingError(800, 300))));
    }
}